...
import com.xczx.media.config.MinioProperties;
...
public class MediaFileServiceImpl extends ServiceImpl<MediaFilesMapper, MediaFiles> implements MediaFileService {
    ...
    public boolean addMediaFilesToMinIO(String localFilePath,String mimeType,String bucket, String objectName) {
        try {
            ...
            XCZXException.cast("上传文件到文件系统失败");
        }
        return false;
    }
    ...
    public UploadFileResultDTO uploadFile(Long companyId, UploadFileParamsDTO uploadFileParamsDTO, String localFilePath) {
        ...
        //将文件上传到minio
        boolean b = addMediaFilesToMinIO(localFilePath, mimeType, minioProperties.getBucket(), objectName);
        //文件大小
        uploadFileParamsDTO.setFileSize(file.length());
        //将文件信息存储到数据库
        MediaFiles mediaFiles = proxy.addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDTO, minioProperties.getBucket(), objectName);
        //准备返回数据
